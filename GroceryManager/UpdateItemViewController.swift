...
class UpdateItemViewController: UIViewController {
    ...
    let units = ["Kg", "Litre", "Pkd"]
    var viewModel: UpdateItemViewModel!
    ...
    override func viewDidLoad() {
        ...
        viewModel = UpdateItemViewModel(reloadDataCallback:reloadData)
        setUpData()
    }
    func setUpData() {
        self.titleTxtFld.text = item?.title
        self.quantityTxtFld.text = item?.quantity
        self.unitTxtFld.text = item?.unit
    }
    ...
    func reloadData() {
        ...
        let alert = UIAlertController(title: "Alert", message: "Item Updated", preferredStyle: .alert)
...
extension UpdateItemViewController: UIPickerViewDataSource, UIPickerViewDelegate {
